...
describe('UTC duration helpers', () => {
  describe('sec (seconds)', () => {
    test('minute() returns 60 seconds per minute by default', () => {
      expect(UTC.sec.minute()).toBe(60);
    });

    test('minute(n) returns n * 60 seconds', () => {
      expect(UTC.sec.minute(5)).toBe(300);
    });

    test('hour() returns 3600 seconds by default', () => {
      expect(UTC.sec.hour()).toBe(3600);
    });

    test('hour(n) returns n * 3600 seconds', () => {
      expect(UTC.sec.hour(2)).toBe(7200);
    });

    test('day() returns 86400 seconds by default', () => {
      expect(UTC.sec.day()).toBe(86400);
    });

    test('day(n) returns n * 86400 seconds', () => {
      expect(UTC.sec.day(3)).toBe(259200);
    });
  });

  describe('ms (milliseconds)', () => {
    test('minute() returns 60000 milliseconds by default', () => {
      expect(UTC.ms.minute()).toBe(60000);
    });

    test('minute(n) returns n * 60000 milliseconds', () => {
      expect(UTC.ms.minute(5)).toBe(300000);
    });

    test('hour() returns 3600000 milliseconds by default', () => {
      expect(UTC.ms.hour()).toBe(3600000);
    });

    test('hour(n) returns n * 3600000 milliseconds', () => {
      expect(UTC.ms.hour(2)).toBe(7200000);
    });

    test('day() returns 86400000 milliseconds by default', () => {
      expect(UTC.ms.day()).toBe(86400000);
    });

    test('day(n) returns n * 86400000 milliseconds', () => {
      expect(UTC.ms.day(3)).toBe(259200000);
    });
  });
});
...
describe('fresh()', () => {
  ...
  // Helper to create date strings by ms from now
  function datetime(ms: number): string {
    return new Date(now - ms).toISOString().replace('T', ' ').slice(0, 19);
  }

  test('returns true for date within default 48h freshness window', () => {
    // 1 hour ago (within 48h)
    expect(fresh(datetime(3600 * 1000))).toBe(true);
  });

  test('returns false for date older than default 48h freshness window', () => {
    // 3 days ago (older than 48h)
    expect(fresh(datetime(3 * 24 * 3600 * 1000))).toBe(false);
  });

  test('returns true for date within custom "2h" freshness window', () => {
    // 90 minutes ago (within 2h)
    expect(fresh(datetime(90 * 60 * 1000), '2h')).toBe(true);
  });

  test('returns false for date older than custom "2h" freshness window', () => {
    // 3 hours ago (older than 2h)
    expect(fresh(datetime(3 * 60 * 60 * 1000), '2h')).toBe(false);
  });

  test('handles "3d" freshness window correctly', () => {
    // 2 days ago (within 3d)
    expect(fresh(datetime(2 * 24 * 3600 * 1000), '3d')).toBe(true);
    // 4 days ago (outside 3d)
    expect(fresh(datetime(4 * 24 * 3600 * 1000), '3d')).toBe(false);
  });

  test('returns false for a future date', () => {
    // 1 hour in the future
    expect(fresh(new Date(now + 3600 * 1000))).toBe(false);
  });

  test('accepts numeric timestamps', () => {
    // 30 minutes ago timestamp
    expect(fresh(now - 30 * 60 * 1000)).toBe(true);
  });

  test('accepts Date objects', () => {
    expect(fresh(new Date(now - 10 * 60 * 1000))).toBe(true);
  });

  test('invalid date input uses Date.now() internally', () => {
    const result = fresh('this is not a date'); // should be fresh since it's now
    expect(result).toBe(true);
  });

  test('returns false for null or undefined', () => {
    expect(fresh(null as any)).toBe(false);
    expect(fresh(undefined as any)).toBe(false);
  });

  test('returns false for non-date objects', () => {
    expect(fresh({} as any)).toBe(false);
  });

  test('invalid level input uses default freshness window (48h)', () => {
    // 'invalid-level' passed to toMs() should default to 48h
    // and this date is only 1 second old (1000ms ago), so still fresh
    expect(fresh(datetime(1000), 'invalid-level')).toBe(true);
  });
});
