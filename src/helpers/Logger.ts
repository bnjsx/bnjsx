 ...
 */
type Log = { date: string; message: string };
...
export class Logger {
  ...
  /**
   * Ensures the log file exists by creating an empty file if it doesn't.
   */
  private ensureFile(): Promise<void> {
    return new Promise((resolve, reject) => {
      access(this.path, constants.F_OK)
        .then(resolve)
        .catch((e) => writeFile(this.path, ''))
        .then(resolve)
        .catch(reject);
    });
  }

  private format(content: string): Logs {
    return content
      .split('<-- LOG -->')
  ...
  public get: Getter = {
    ...
    logs: (): Promise<Logs> => {
      return new Promise((resolve, reject) => {
        this.ensureFile()
          .then(() => readFile(this.path, 'utf-8'))
          .then((content) => resolve(this.format(content)))
   ...
   */
  public log(input: string | Error): Promise<void> {
    return new Promise((resolve, reject) => {
      const date = UTC.get.datetime();
      ...
      if (input instanceof Error) {
        ...
        if (typeof input.stack === 'string') {
          ...
          const trace = lines.find((line) => line.startsWith('at '));
          if (trace) location = trace;
        }
      } else if (typeof input === 'string') {
        message = input;
      } else {
        return reject(new LoggerError(`Invalid log input: ${String(input)}`));
      }

      const logLine = `[${date}] [${name}] ${message} ${location}`.trim();
