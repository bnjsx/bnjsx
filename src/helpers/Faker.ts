import { isFullArr, isInt, isNum, isStr, isUndefined } from './Test';
...
export class Faker {
  ...
  /**
   * Generates an array of sequential numeric IDs within a specified range.
   *
   * @param start - The starting number of the range (inclusive).
   * @param end - The ending number of the range (inclusive).
   * @throws `FakerError` If `start` or `end` are not numbers or if `start` is greater than `end`.
   * @returns An array of numbers from `start` to `end`, inclusive.
   */
  public ids(start: number, end: number): number[] {
    if (!isNum(start) || !isNum(end) || start > end) {
      throw new FakerError('Invalid arguments provided');
    }
